...
// Find the return statement of Dashboard
...
// remove the progress bar from Dashboard.jsx
const newReturn = `return (
    <>
      <div className="dash-universe" style={{ 
        height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', 
        padding: '20px', gap: '20px', background: 'var(--bg-primary)', boxSizing: 'border-box' 
      }}>
        ...
        <header style={{ 
          ...
          borderRadius: '24px', flexShrink: 0, boxShadow: 'var(--card-shadow)',
          backdropFilter: 'blur(36px) saturate(160%)', WebkitBackdropFilter: 'blur(36px) saturate(160%)'
        }}>
          {/* Top Row */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 1.5rem', borderBottom: '1px solid var(--glass-border)' }}>
            ...
            {isStudent && (
              <>
                ...
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 16px', borderRadius: '100px', border: '1px solid transparent', background: activeTab === 'notifications' ? 'var(--accent-glow)' : 'transparent', color: activeTab === 'notifications' ? 'var(--accent-secondary)' : 'var(--text-muted)', fontWeight: activeTab === 'notifications' ? 600 : 500, cursor: 'pointer', whiteSpace: 'nowrap', transition: 'all 0.2s', position: 'relative' }}
                >
        ...
        <main className="dash-main" style={{ 
          flex: 1, background: 'var(--card-bg-solid)', border: '1px solid var(--glass-border)',
          borderRadius: '24px', overflowY: 'auto', display: 'flex', flexDirection: 'column',
          boxShadow: 'var(--card-shadow)', padding: '24px'
        }}>
          {activeTab === 'profile' && (
            <header className="dash-header" style={{ marginBottom: '20px' }}>
              <div>
                <h1 style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--text-main)' }}>Profile Management</h1>
                <p style={{ color: 'var(--text-muted)' }}>Manage your account credentials and contact data.</p>
              </div>
              {!editMode ? (
                <button className="btn-edit" onClick={() => setEditMode(true)} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'var(--accent-glow)', color: 'var(--accent-secondary)', padding: '8px 16px', borderRadius: '10px', border: 'none', cursor: 'pointer', fontWeight: 600 }}><Edit2 size={16} /> Edit Profile</button>
              ) : (
                <button className="btn-cancel" onClick={() => { setEditMode(false); setFormData(profile); setPasswordData({ currentPassword: '', newPassword: '', confirmPassword: '' }); setMessage({ text: '', type: '' }); }} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'rgba(239,68,68,0.1)', color: '#ef4444', padding: '8px 16px', borderRadius: '10px', border: 'none', cursor: 'pointer', fontWeight: 600 }}><X size={16} /> Cancel</button>
              )}
            </header>
          )}

          <div className="dash-content-area" style={{ flex: 1 }}>
            {message.text && (
                ...
                {!editMode ? (
                  ...
                  <div className="profile-grid">
                    ...
                    <div className="profile-card">
                      <h3 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-main)', marginBottom: '15px' }}>Core Identification</h3>
                    ...
                    <div className="profile-card">
                      <h3 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-main)', marginBottom: '15px' }}>Geospatial Data</h3>
                    ...
                    <div className={\`profile-card \${!isPartner ? 'full-width' : ''}\`}>
                      <h3 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-main)', marginBottom: '15px' }}>Contact Details</h3>
                    ...
                    {isPartner && (
                      <div className="profile-card">
                        <h3 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-main)', marginBottom: '15px' }}>Business Details</h3>
...
// Find floating chat area start to keep it
...
console.log("Replaced Dashboard.jsx layout");

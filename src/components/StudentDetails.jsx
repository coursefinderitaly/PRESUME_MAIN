...
import { ArrowLeft, ArrowRight, Save, User, FileText, FolderOpen, Mail, Phone, MapPin, Eraser, X, UploadCloud, Eye, AlertTriangle, ChevronLeft, ChevronRight, Globe, CheckSquare, Plus, Trash2, Download, CheckCircle, Search, ExternalLink, GraduationCap, Building2 } from 'lucide-react';
  ...
  return (
    <div className="view-standard student-details-wrapper" style={{ animation: 'fadeIn 0.3s ease', display: 'flex', flexDirection: 'column', height: '100%', flex: 1, minHeight: 0 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexShrink: 0, padding: '4px 8px 12px 4px', borderBottom: '1px solid var(--glass-border)', animation: 'fadeUp 0.3s ease' }}>
        <button 
          onClick={goBack} 
          style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'rgba(255,255,255,0.05)', border: '1px solid var(--glass-border)', color: 'var(--text-main)', cursor: 'pointer', padding: '8px 14px', borderRadius: '10px', transition: 'all 0.2s', fontSize: '0.85rem', fontWeight: 600 }} 
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px' }}>
          <h2 style={{ fontSize: '1.2rem', fontWeight: 800, margin: 0, color: 'var(--text-main)', letterSpacing: '-0.01em' }}>
            {student.firstName} {student.lastName}
          </h2>
          <span style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-dim)', fontSize: '0.85rem' }}>
            <Mail size={12} /> {student.email}
          </span>
        </div>

        <div style={{ marginLeft: 'auto', display: 'flex', gap: '20px', alignItems: 'center', fontSize: '0.8rem', fontWeight: 600 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <span style={{ color: 'var(--text-muted)', textTransform: 'uppercase', fontSize: '0.7rem', letterSpacing: '0.05em' }}>Offer:</span>
            <span style={{ color: 'var(--accent-secondary)', background: 'var(--accent-glow)', padding: '3px 8px', borderRadius: '6px', border: '1px solid var(--glass-border)' }}>
              {student.offerStatus || 'Pending'}
            </span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <span style={{ color: 'var(--text-muted)', textTransform: 'uppercase', fontSize: '0.7rem', letterSpacing: '0.05em' }}>Status:</span>
            <span style={{ color: 'var(--accent-primary)', background: 'var(--accent-glow)', padding: '3px 8px', borderRadius: '6px', border: '1px solid var(--glass-border)' }}>
          ...
          {[
            ...
            return (
              <button
                ...
                style={{
                  background: isActive ? 'var(--card-bg-solid)' : 'transparent',
                  ...
                  transition: 'all 0.2s',
                  textAlign: 'left',
                  boxShadow: isActive ? '0 8px 20px -8px rgba(0,0,0,0.3)' : 'none'
                }}
              >
                <div style={{
                  background: isActive ? 'var(--accent-glow)' : 'var(--glass-bg)',
                  color: isActive ? 'var(--accent-primary)' : 'var(--text-muted)',
                  padding: '10px', borderRadius: '12px', flexShrink: 0, transition: 'all 0.2s'
                }}>
                  <IconCmp size={18} />
                </div>
                <div>
                  <div style={{ fontSize: '0.95rem', fontWeight: 700, color: isActive ? 'var(--text-main)' : 'var(--text-muted)' }}>{tabObj.label}</div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-dim)', marginTop: '2px' }}>{tabObj.desc}</div>
                </div>
              </button>
            )
          })}
        ...
        <div className="tab-content" style={{
          ...
          <div style={{ display: activeTab === 'profile' ? 'block' : 'none' }}>
            <form onSubmit={handleSaveProfile} className="edit-form-grid">
              ...
              {/* Actions */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '20px', paddingRight: '80px' }}>
              ...
              <div style={{ background: 'var(--card-bg-solid)', backdropFilter: 'blur(40px)', width: '92%', maxWidth: '1400px', height: '90vh', borderRadius: '24px', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 30px 60px -12px rgba(0, 0, 0, 0.5)', border: '1px solid var(--glass-border)', position: 'relative' }}>
                {!isSubmitted ? (
                  <>
                    ...
                    <div style={{ padding: '10px 80px 10px 30px', borderTop: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>

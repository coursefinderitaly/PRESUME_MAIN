  ...
  const content = (
    <>
      <motion.div 
        ...
        className={`relative w-full ${isEmbedded ? "max-w-3xl" : "max-w-4xl"} bg-white/[0.03] backdrop-blur-2xl rounded-[32px] md:rounded-[40px] shadow-[0_32px_80px_-20px_rgba(0,0,0,0.5)] p-4 sm:p-8 border border-white/10 overflow-hidden group mx-auto`}
      ...
      >
        ...
        {/* 🛸 INTEGRATED TEXT OVERLAY - Moved Inside Canvas */}
        <div className="absolute top-6 md:top-10 left-6 md:left-10 z-20 flex flex-col pointer-events-none max-w-[240px] sm:max-w-[300px] text-left">
          <div className="inline-flex self-start items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 backdrop-blur-md text-[9px] font-black tracking-widest uppercase text-accent-gold mb-3">
            <Sparkles size={10} /> Explore Regions
          </div>
          <h1 className="text-2xl sm:text-4xl font-black tracking-tighter mb-2 text-white leading-tight drop-shadow-xl">
            Italian <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent-gold to-yellow-300">Universities</span>
          </h1>
          <p className="text-gray-400 text-xs font-medium leading-relaxed drop-shadow-md">
            Interactive visual overview of premier public educational institutions.
          </p>
        </div>

        {/* Dynamic Hover Tooltip - Moved to Bottom-Left or Bottom-Right for layout synergy */}
        <div className="absolute bottom-6 left-6 md:bottom-10 md:left-10 flex items-center z-20 pointer-events-none">
          <AnimatePresence mode="wait">
            {hoveredRegion ? (
              <motion.div
                ...
                className="bg-white/10 backdrop-blur-xl border border-white/10 text-white px-4 py-2 rounded-2xl text-xs font-black flex items-center gap-3 shadow-2xl uppercase tracking-wider"
              >
              ...
              <motion.div
                 ...
                 animate={{ opacity: 1 }}
                 exit={{ opacity: 0 }}
                 className="bg-black/20 backdrop-blur-md border border-white/5 text-gray-400 text-[9px] px-3 py-1.5 rounded-lg font-bold uppercase tracking-widest flex items-center gap-2"
              >
        ...
        <ComposableMap
          ...
          projectionConfig={{
            scale: isEmbedded ? 2000 : 2200, // Shrunk substantially to fit vertically
            center: [12.8, 42.2], // Slightly optimized vertical alignment
          }}
          className="w-full h-auto outline-none drop-shadow-[0_15px_30px_rgba(0,0,0,0.4)]"
          style={{ maxWidth: '100%', maxHeight: '520px', margin: '0 auto' }}
        >
